...
specs = do
  ...
  it "data is retained" $ bstValue (singleton int4) `shouldBe` Just 4
  ...
  it "empty list to tree" $ fromList noInts `shouldBe` empty

  it "empty list has no value" $ bstValue (fromList noInts) `shouldBe` Nothing
  ...
  it "empty tree to list" $ length (toList empty) `shouldBe` 0

  it "iterating one element" $ toList (singleton int4) `shouldBe` [4]

  it "iterating over smaller element"
    $          toList (fromList [int4, 2])
    `shouldBe` [2, 4]

  it "iterating over equal element"
    $          toList (fromList [int4, 4])
    `shouldBe` [4, 4]

  it "iterating over larger element"
    $          toList (fromList [int4, 5])
    `shouldBe` [4, 5]

  it "iterating over complex tree"
    $          toList (fromList [int4, 2, 1, 3, 6, 7, 5])
    `shouldBe` [1 .. 7]
