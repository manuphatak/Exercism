...
specs = do
  ...
  describe "ability" $ it "generates values within range" $ property
    (forAll ability abilityWithinRange)

  describe "character" $ it "generates valid characters" $ property
    (forAll character characterIsValid)
 where
  test Case {..} = it description assertion
   where
    description =
      printf "computes the modifier for %2d to be %2d" input expected
  ...
  abilityWithinRange' character' (name, ability') = counterexample
    msg
    (abilityWithinRange (ability' character'))
    where msg = "The '" ++ name ++ "' ability is out of range"

  hitpointsAddUp :: Character -> Property
  hitpointsAddUp Character {..} = counterexample
    msg
