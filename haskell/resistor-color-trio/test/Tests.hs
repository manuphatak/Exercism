...
specs = do
 ...
 where
  testLabel Case {..} =
   ...
   where
    description Resistor {..} = show bands ++ " should give " ++ expectedLabel

  testOhms Case {..} =
    it (description input) $ ohms input `shouldBe` expectedOhms
   where
...
cases :: [Case]
cases =
  [ Case { input         = Resistor (Black, Black, Black)
