...
import           Test.QuickCheck                ( arbitraryASCIIChar
                                                , conjoin
                                                , counterexample
                                                , discard
                                                , elements
...
specs = describe "diamond" $ do
  ...
  it "should have an odd number of rows" $ forAllDiamond $ odd . length

  it "should have equal top and bottom" $ forAllDiamond $ \rows ->
    let halfRoundDown = length rows `div` 2
    in  take halfRoundDown rows === take halfRoundDown (reverse rows)

  it "should have the same width and height" $ forAllDiamond $ \rows ->
 ...
 where
  test Case {..} = it description assertion
   where
    assertion =
      (fmap . fmap) convertString (diamond input) `shouldBe` Just expected
